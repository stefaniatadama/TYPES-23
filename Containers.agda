...
record Container (C : Category ℓ ℓ') : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
  constructor _◁_&_ 
  field
    S : Type ℓ' 
    P : S → ob C
...
module Conts {C : Category ℓ ℓ'} where
    ...
    -- Category Cont with objects of type Container C

    record _⇒c_ (C₁ C₂ : Container C) : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
      constructor _◁_
      field
        u : S C₁ → S C₂
    ...
    id-c : {Con : Container C} → Con ⇒c Con
    id-c = (λ s → s) ◁ λ _ → C .id

    _⋆c_ : {C₁ C₂ C₃ : Container C} → C₁ ⇒c C₂ → C₂ ⇒c C₃ → C₁ ⇒c C₃
    _⋆c_ (u ◁ f) (v ◁ g) = (λ a → v (u a)) ◁ λ a → (g (u a)) ⋆⟨ C ⟩ (f a)

    assoc-c : {C₁ C₂ C₃ C₄ : Container C} (f : C₁ ⇒c C₂) (g : C₂ ⇒c C₃) (h : C₃ ⇒c C₄) →
              (f ⋆c g) ⋆c h ≡ f ⋆c (g ⋆c h)
    assoc-c (u ◁ f) (v ◁ g) (w ◁ h) i = (λ a → w (v (u a))) ◁ λ a → C .⋆Assoc (h (v (u a))) (g (u a)) (f a) (~ i)

    isSet⇒c : {C₁ C₂ : Container C} → isSet (C₁ ⇒c C₂)
    ...
    ⟦_⟧-mor : {C₁ C₂ : Container C} → C₁ ⇒c C₂ → NatTrans ⟦ C₁ ⟧-obj ⟦ C₂ ⟧-obj
    N-ob (⟦_⟧-mor (u ◁ f)) X (s , p) = u s , ((f s) ⋆⟨ C ⟩ p)
    N-hom (⟦_⟧-mor (u ◁ f)) xy i (a , b) = u a , C .⋆Assoc (f a) b xy (~ i)
    
    ⟦_⟧-id : {C₁ : Container C} → ⟦_⟧-mor {C₁} {C₁} (id-c {C₁}) ≡ idTrans ⟦ C₁ ⟧-obj
    ...
    ⟦_⟧-comp : {U V W : Container C} (g : U ⇒c V) (h : V ⇒c W) →
